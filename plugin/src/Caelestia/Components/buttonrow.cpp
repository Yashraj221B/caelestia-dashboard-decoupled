...
void ButtonRow::relayout() {
    ...
    const auto totalSpacing =
        static_cast<qreal>(nChildren - 1) * m_spacing;
    ...
    const auto widthPerItem =
        (width() - totalSpacing - reservedWidth) /
    ...
    for (auto* const child : validChildren) {
        baseWidths.append(
            child->property("fillWidth").toBool()
                ? widthPerItem
                : child->implicitWidth()
        );
    }

    // First calculate all final widths so we know the
    // total width of the button group.
    qreal contentWidth = totalSpacing;

    QList<qreal> finalWidths;
    finalWidths.reserve(nChildren);
    ...
    for (int i = 0; i < nChildren; ++i) {
        ...
        auto prevExtraWidth =
            i > 0
                ? getMorphExpansion(validChildren[i - 1])
                : 0.0;
        ...
        // Items at edges push by full amount,
        // items in the middle push by half.
        if (i > 1)
            prevExtraWidth /= 2;
        ...
        const auto finalWidth =
            baseWidths[i]
            + getMorphExpansion(child)
            - prevExtraWidth
            - nextExtraWidth;

        finalWidths.append(finalWidth);
        contentWidth += finalWidth;
    }

    // Center the entire group inside ButtonRow.
    qreal accX = qMax(0.0, (width() - contentWidth) / 2.0);

    for (int i = 0; i < nChildren; ++i) {
        auto* const child = validChildren[i];

        child->setWidth(finalWidths[i]);
